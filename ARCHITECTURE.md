...
* **Monitoring depth should scale with task risk, not be uniform.** A small, well-scoped delegation only needs a final `kilo_task_result` check. A large, multi-file, or high-stakes one deserves to be watched while it runs — so two tools exist at different fidelities:
  ...
  * `kilo_task_progress` — reads Kilo's *own* live plan straight from its session database (`~/.local/share/kilo/kilo.db`): the `todo` table (Kilo's per-step plan with status pending/in_progress/completed — the same structure the TUI shows the user), the `part` table (a tail of Kilo's actual streamed commentary), and the `session` row (running cost/tokens). This is real content, not an inference from process activity, and required no changes to how `kilo run` itself is invoked — the DB is a read-only side channel already used by `kilo_task_status`, just queried more deeply.
* **Intervention, not just observation.** Two levers close the loop once a problem is spotted:
  * `kilo_task_cancel` — kills the tracked subprocess by PID (SIGTERM → SIGKILL after a grace period). The PID is captured directly from `_spawn_kilo_background`'s return value at launch time (see [§8](#8-background-execution-reliability-detached-processes) — this replaced an earlier `on_start`-callback/`asyncio.create_task` design), so cancellation never depends on re-discovering the process via `pgrep`. Since there is no longer an in-process runner coroutine that could race a completion write against the cancel, the "cancelled" verdict written by `kilo_task_cancel` simply can't be clobbered after the fact — the only other writer is the lazy reconcile path, which already short-circuits on any non-`"running"` status.
  * `continue_session_id` on `kilo_implement` — passes `kilo run --session <id>` to resume an existing session instead of starting fresh. Confirmed against the actual CLI: Kilo queues a follow-up prompt into a session even while a previous run in that session is still executing (`KiloSessionPromptQueue`), so this genuinely works as a corrective steer, not just a resume-after-the-fact. The session id is resolved automatically (matched in `kilo.db` by workspace path and start time) and surfaced in every result/progress report, so the caller never has to hunt for it.
* **What was deliberately left out:** Kilo also exposes a `POST /session/{id}/abort` HTTP endpoint (graceful in-session interrupt) and a `--format json` streaming mode for `kilo run`, both discovered while researching this feature. Neither is used here — the abort endpoint requires running `kilo serve` and driving its SDK, and `--format json` would change `kilo run`'s stdout shape enough to break `_parse_final_report`'s Final-Report parsing. The `kilo.db` read-only side channel plus PID-based hard-kill covers the monitoring/intervention need without that added surface area; it can be revisited if a future need specifically requires graceful in-session abort.
