...
| `kilo_orchestrator_spawn_agent`     | ✅ Implemented — `kilo_implement(background=true)`, the default. Since 2026-07-29 it runs as a genuinely detached OS process (`setsid`), no longer tied to the MCP server's lifecycle (this was a real bug: tasks stuck on "running" forever — 3 distinct causes, all fixed and verified live, see `ARCHITECTURE.md` §8). |
| `kilo_orchestrator_list_sessions`   | ⚠️ Partial — `kilo_task_status` (OS-level heuristic, all `kilo run` processes) and `kilo_task_progress` (live plan/commentary for a known `task_id`) cover most of the use case; a single tool listing *all* sessions with an attached `git diff --stat` in one call is still missing.                                     |
| `kilo_orchestrator_prompt_agent`    | ✅ Implemented — `kilo_implement(continue_session_id=...)`, verified against the real CLI (Kilo queues the follow-up even against a still-running session).                                                                                                                                                                    |
| `kilo_orchestrator_get_logs`        | ⚠️ Partial — `kilo_task_progress` returns a tail of recent commentary from `kilo.db`; the full raw log exists on disk (`TASKS_DIR/<task_id>.log`, introduced with the 2026-07-29 fix) but isn't yet exposed by a dedicated tool.                                                                                             |
